module Mindra.Gloss.ParserTest where

import Data.Either (isLeft, isRight)

import qualified Text.RawString.QQ as QQ

import Mindra.Gloss.Parser (parseConfiguration, parsePicture)

import Test.Tasty
import Test.Tasty.HUnit

tests :: TestTree
tests = testGroup "Gloss Parser Tests" [unitTests]

unitTests :: TestTree
unitTests = testGroup
  "Unit Tests for Gloss Parser"
  [ testCase "parseConfiguration" $ do
    assertBool "Valid case -- default configuration" $ isRight $ parseConfiguration "Gloss"
    assertBool "Valid case -- static mode" $ isRight $ parseConfiguration "Gloss Mode 0"
    assertBool "Valid case -- interactive mode" $ isRight $ parseConfiguration "Gloss Mode 1"
    assertBool "Valid case -- with StepsPerSecond" $ isRight $ parseConfiguration
      "Gloss StepsPerSecond 100"
    assertBool "Valid case -- with Color" $ isRight $ parseConfiguration "Gloss Color 200 0 200 255"
    assertBool "Valid case -- with NoEvent" $ isRight $ parseConfiguration "Gloss NoEvent"
    assertBool "Valid case -- with NoStep" $ isRight $ parseConfiguration "Gloss NoStep"
    assertBool "Valid case -- with NoEvent and NoStep" $ isRight $ parseConfiguration
      "Gloss NoEvent NoStep"
    assertBool "Valid case -- with full configuration" $ isRight $ parseConfiguration
      "Gloss Mode 0 Window 200 400 10 15 \"Title\" Color 200 0 0 200 NoEvent NoStep"
    assertBool "Valid case -- with full configuration" $ isRight $ parseConfiguration [QQ.r|
        Gloss
        Window 200 400 10 10 "Window title"
        Color 200 0 200 255
        NoEvent
        NoStep
        Mode 0
        |]
  , testCase "parsePicture" $ do
    assertBool "Line" $ isRight $ parsePicture "Line [[0 0] [10 10]]"
    assertBool "Circle" $ isRight $ parsePicture "Circle 10"
    assertBool "Big Test" $ isRight $ parsePicture [QQ.r|
        [ Circle 10
          Line [[0 0] [5 5]]
          Circle 100
        ]
        |]
    assertBool "Big Test -- draw-basic" $ isRight $ parsePicture [QQ.r|
